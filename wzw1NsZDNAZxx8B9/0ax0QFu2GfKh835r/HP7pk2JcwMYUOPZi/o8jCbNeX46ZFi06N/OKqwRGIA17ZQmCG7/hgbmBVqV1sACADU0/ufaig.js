function formatMsgTime(date) {
  ...
  if (isThisYear) return date.toLocaleDateString([], {
    month: "short",
    day: "numeric"
  });
...
const MEDIA_GROUP_GAP_MS = 5 * 60 * 1e3;
...
function groupMessages(messages) {
  ...
  messages.forEach(msg => {
    ...
    if (currentGroup && currentGroup.sender_id === msg.sender_id) {
      ...
      const withinWindow = gap <= MEDIA_GROUP_GAP_MS;
...
function makeGroupAvatar(contact, senderId) {
  ...
  const sender = contact && contact.isGroup && senderId ? fluxContacts.find(c => c.id === senderId) || null : contact;
  const avatarUrl = sender?.avatarUrl || sender?.avatar_url || null;
  av.className = "flux-group-avatar";
  if (senderId) {
    av.dataset.senderId = senderId;
    av.style.cursor = "pointer";
    av.onclick = e => {
      e.stopPropagation();
      openNicknamePanel(senderId);
    };
  }
  if (avatarUrl) {
...
function fixSeenLabels(container) {}
...
function applyBubbleGrouping(bubblesWrap) {
  ...
  const children = Array.from(bubblesWrap.children).filter(el => el.classList.contains("flux-bubble-outer") || el.classList.contains("flux-bubble-wrap"));
...
function makeMediaCollageBubble(mediaGroup, isSent, contact, currentUserId) {
  ...
  replyBtn.className = "flux-reply-btn";
  ...
  replyBtn.onclick = e => {
    e.stopPropagation();
    startReply("Photo", isSent ? profile.username ? profile.username.slice(1) : "me" : _fluxGroupMsgSenderName(contact, mediaGroup.sender_id) || (contact ? contact.username || contact.realName || contact.name : "User"), mediaGroup.id, urls[0]);
  };
  const moreBtn = document.createElement("button");
  moreBtn.className = "flux-more-btn";
  ...
  if (isSent) {
    bWrap.appendChild(moreBtn);
    bWrap.appendChild(replyBtn);
    bWrap.appendChild(collage);
  } else {
    bWrap.appendChild(collage);
    bWrap.appendChild(replyBtn);
    bWrap.appendChild(moreBtn);
...
function _recollapseMediaInBubbles(bubblesWrap, isSent, contact, currentUserId) {
  ...
  children.forEach(child => {
    ...
    if (collage) {
      ...
      storedUrls.forEach(url => msgs.push({
        _url: url,
        _el: child
      }));
      return;
  ...
  while (i < msgs.length) {
    ...
    if (m._url) {
      const run = [ m._url ];
      while (i + 1 < msgs.length && msgs[i + 1]._url) {
        i++;
        run.push(msgs[i]._url);
      }
      if (run.length >= 2) {
        const fakeGroup = {
          urls: run,
        ...
        replyBtn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"><polyline points="9 17 4 12 9 7"/><path d="M20 18v-2a4 4 0 0 0-4-4H4"/></svg>`;
        if (isSent) {
          singleBWrap.appendChild(bubble);
        } else {
          singleBWrap.appendChild(bubble);
        }
        const bOuter = document.createElement("div");
...
function addSwipeToReply(bWrap, isSent, onReply) {
  ...
  bWrap.addEventListener("touchmove", e => {
    ...
    dragging = true;
    e.preventDefault();
    const clamped = isSent ? Math.max(-MAX_DRAG, Math.min(0, dx)) : Math.min(MAX_DRAG, Math.max(0, dx));
    curX = clamped;
  ...
  bWrap.addEventListener("touchend", endSwipe, {
    passive: true
  });
  bWrap.addEventListener("touchcancel", endSwipe, {
    passive: true
  });
}
...
function makeBubbleWrap(msg, isSent, contact, currentUserId) {
  ...
  replyBtn.onclick = e => {
    e.stopPropagation();
    startReply(msg.media_url && !msgText ? "Photo" : msgText, msgSender, msg.id, msg.media_url || null);
  };
  const moreBtn = document.createElement("button");
  moreBtn.className = "flux-more-btn";
  moreBtn.title = "More options";
  ...
  moreBtn.onclick = e => {
    e.stopPropagation();
    showMsgActionMenu(e, bWrap, msg, isSent);
  };
  const bubble = document.createElement("div");
  ...
  if (msg.reply_to_text) {
    ...
    rp.onclick = e => {
      e.stopPropagation();
      scrollToReplyTarget(msg.reply_to_id, msg.reply_to_text, msg.reply_to_author || "");
    };
    bubble.prepend(rp);
  }
  if (msg.media_url && !msg.content) {
    ...
    bubble.classList.add("flux-bubble-media-caption");
    let mediaEl;
    if (msg.is_video) {
      ...
      mediaEl.onclick = function() {
        openLightbox(this.src, [ this.src ]);
      };
    }
    mediaEl.className = "flux-bubble-caption-media";
    bubble.appendChild(mediaEl);
    if (msg.content && msg.content.trim()) {
      const textNode = document.createElement("div");
      textNode.className = "flux-bubble-caption-text";
  ...
  bubble.addEventListener("contextmenu", e => {
    e.preventDefault();
    showCtxMenu(e.clientX, e.clientY, bWrap, msg, isSent, contact);
  });
  if (isSent) {
    bWrap.appendChild(moreBtn);
    bWrap.appendChild(replyBtn);
...
async function addReaction(bWrap, emoji) {
  ...
  if (!user) return;
  let reactions = _reactionCache[msgId] ? {
    ..._reactionCache[msgId]
  } : {};
  ...
  supabaseClient.from("messages").update({
    reactions: reactions
  }).eq("id", msgId).then(({error: error}) => {
    if (error) {
      delete _reactionCache[msgId];
      supabaseClient.from("messages").select("reactions").eq("id", msgId).single().then(({data: data}) => {
        if (data) {
...
function showReactionPicker(bWrap) {
  closeAllFluxDropdowns(closeReactionPicker);
  closeReactionPicker();
  const picker = document.createElement("div");
  picker.id = "flux-reaction-picker-popup";
  picker.style.cssText = "position:fixed;z-index:10001;background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:8px 10px;display:flex;gap:6px;box-shadow:0 8px 24px rgba(0,0,0,0.18);animation:popIn 0.15s ease;";
...
function showCtxMenu(x, y, bWrap, msg, isSent, contact) {
  closeAllFluxDropdowns(closeCtxMenu);
  ...
  replyBtn.onclick = () => {
    startReply(text, sender, msg ? msg.id : null);
    closeCtxMenu();
  };
  const reactBtn = document.createElement("button");
  reactBtn.className = "flux-ctx-item";
  reactBtn.innerHTML = `😊 React`;
  reactBtn.onclick = () => {
    closeCtxMenu();
    showReactionPicker(bWrap);
  };
...
async function deleteSingleCollageImage(bWrap, urlToDelete, mediaGroup, isSent, contact, currentUserId) {
  ...
  const newMediaGroup = {
    urls: newUrls,
    msgs: newMsgs,
    id: mediaGroup.id || newMsgs[0] && newMsgs[0].id || ""
  };
...
function closeCtxMenu() {
  document.getElementById("fluxCtxMenu").style.display = "none";
}
...
function startReply(text, author, msgId, imageUrl) {
  ...
  if (bar) {
    ...
    document.getElementById("fluxReplyBarAuthor").textContent = author;
    document.getElementById("fluxReplyBarText").textContent = text;
    _setReplyBarThumb("fluxReplyBarThumb", imageUrl);
  }
  const fsBar = document.getElementById("fluxFsReplyBar");
  if (fsBar) {
    fsBar.classList.add("show");
    document.getElementById("fluxFsReplyBarAuthor").textContent = author;
    document.getElementById("fluxFsReplyBarText").textContent = text;
...
async function scrollToReplyTarget(replyMsgId, replyText, replyAuthor) {
  console.log("[scrollToReply] called", {
  ...
  function flashWrap(wrap) {
    ...
    wrap.classList.remove("flux-reply-jump-highlight");
    void wrap.offsetWidth;
    wrap.classList.add("flux-reply-jump-highlight");
    setTimeout(() => wrap.classList.remove("flux-reply-jump-highlight"), 1400);
  }
  function findInDOM() {
  ...
  if (_findFill) {
    _findFill.dataset.origBg = _findFill.style.background;
    _findFill.dataset.origShadow = _findFill.style.boxShadow;
    _findFill.style.background = _findIsDark ? "#ffffff" : "#000000";
    _findFill.style.boxShadow = "none";
  }
  try {
    ...
    if (!targetMsg) {
      _finishLoadBar(_findBarId, _findFillId);
      setTimeout(() => {
        if (_findFill) {
          _findFill.style.background = _findFill.dataset.origBg || "";
          _findFill.style.boxShadow = _findFill.dataset.origShadow || "";
        }
      }, 560);
      return;
    }
    ...
    const {count: posCount} = await _fluxApplyConvFilter(supabaseClient.from("messages").select("*", {
      count: "exact",
      head: true
    }), userId, user.id, _replyConvIsGroup).lte("created_at", targetMsg.created_at);
    ...
    const CONTEXT = 5;
    const newStart = Math.max(0, posCount - 1 - CONTEXT);
    ...
    const {data: gapData} = await _fluxApplyConvFilter(supabaseClient.from("messages").select("*"), userId, user.id, _replyConvIsGroup).order("created_at", {
      ascending: true
    ...
    const groups = groupMessages(gapData.map(m => ({
      ...m,
      ts: m.created_at
    })));
    ...
    groups.forEach(item => {
      if (item.type === "separator") {
        ...
        item.messages.forEach(msg => {
          if (msg.id) renderedMsgIds.add(msg.id);
          bubblesWrap.appendChild(makeBubbleWrap(msg, isSent, contact, user.id));
        });
        applyBubbleGrouping(bubblesWrap);
        rowEl.appendChild(bubblesWrap);
        groupEl.appendChild(rowEl);
