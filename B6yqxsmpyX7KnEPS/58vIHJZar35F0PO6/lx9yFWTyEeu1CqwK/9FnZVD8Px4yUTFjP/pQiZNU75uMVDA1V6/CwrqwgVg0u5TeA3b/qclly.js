...
async function _loadMobProfileSheet(targetId) {
  ...
  const [{data: profileData}, {data: nicknameRows}] = await Promise.all([ supabaseClient.from("profiles").select("username, display_name, avatar_url").eq("id", targetId).single(), supabaseClient.from("nicknames").select("nickname").eq("setter_id", user.id).eq("target_id", targetId).order("created_at", {
    ascending: false
  }).limit(1) ]);
...
function mobShowNicknameEditor() {
  ...
  if (input) {
    input.value = activeFluxNickname || "";
    input.focus();
  }
...
async function _saveNicknameValue(nickValue) {
  ...
  if (nickValue) {
    await supabaseClient.from("nicknames").upsert({
      setter_id: user.id,
      target_id: activeFluxProfileTarget,
      nickname: nickValue
    }, {
...
async function mobCommitNickname() {
  ...
  if (contact) {
    ...
    contact.initials = displayName[0].toUpperCase();
    buildFLUXConvList();
  }
}
...
async function mobClearNickname() {
  ...
  if (contact) {
    ...
    contact.initials = profileName[0].toUpperCase();
    buildFLUXConvList();
  }
}
...
function mobRenderThemePicker() {
  ...
  (typeof RELAY_THEMES !== "undefined" ? RELAY_THEMES : []).forEach(theme => {
    ...
    item.onclick = () => {
      selectRelayTheme(theme.id);
      mobRenderThemePicker();
    };
    item.innerHTML = '<div class="flux-theme-preview">' + '<span class="flux-theme-bubble-sent" style="background:' + theme.sent + ";color:" + theme.sentText + ';">Hey!</span>' + '<span class="flux-theme-bubble-recv" style="background:' + theme.recv + ";color:" + theme.recvText + ';">Hello &#128075;</span>' + "</div>" + "<div>" + '<div class="flux-theme-item-label">' + theme.tag + "</div>" + '<div class="flux-theme-item-sub">' + (theme.sub || "") + "</div>" + "</div>";
