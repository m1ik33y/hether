...
function makeBubbleWrap(msg, isSent, contact, currentUserId) {
  ...
  if (msg.media_url && !msg.content) {
    ...
    // Tagged-text (caption) message: media wrapped inside the same text
    // bubble, with the caption below it — like a normal text bubble that
    // happens to have media pinned to its top.
    ...
    if (msg.is_video) {
      ...
      mediaEl.src = msg.media_url;
      mediaEl.onclick = function () { openLightbox(this.src, [this.src]); };
    }
...
async function scrollToReplyTarget(replyMsgId, replyText, replyAuthor) {
  ...
  function flashWrap(wrap) {
    ...
    // this reply target was just flashed a moment ago.
