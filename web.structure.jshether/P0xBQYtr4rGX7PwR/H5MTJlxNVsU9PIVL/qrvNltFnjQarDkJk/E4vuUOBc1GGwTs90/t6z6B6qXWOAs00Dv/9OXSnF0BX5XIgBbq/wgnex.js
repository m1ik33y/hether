(function() {
  ...
  let termHistIdx = -1;
  function termEl() {
    return document.getElementById("vaultTerminal");
  }
  function bodyEl() {
    return document.getElementById("termBody");
  }
  function inputEl() {
    return document.getElementById("termLiveInput");
  }
  function inputLineEl() {
    return document.querySelector("#termBody .term-input-line");
  }
  ...
  async function cmdClearRelay(u1, u2) {
    ...
    const {data: myProfile, error: roleErr} = await supabaseClient.from("profiles").select("role").eq("id", currentUser.id).single();
    ...
    const {data: p1Data, error: p1Err} = await supabaseClient.from("profiles").select("id, username").eq("username", u1).maybeSingle();
    if (p1Err) {
    ...
    const {data: p2Data, error: p2Err} = await supabaseClient.from("profiles").select("id, username").eq("username", u2).maybeSingle();
    if (p2Err) {
    ...
    const {error: delErr, count: count} = await supabaseClient.from("messages").delete({
      count: "exact"
      ...
      for (const [ch, notifyId, clearerId] of [ [ ch1, id1, id2 ], [ ch2, id2, id1 ] ]) {
        ch.subscribe(async status => {
          if (status === "SUBSCRIBED") {
            ...
            setTimeout(() => supabaseClient.removeChannel(ch), 2e3);
          }
        });
      }
    } catch (e) {
  ...
  async function cmdSetMediaPerm(u, enable) {
    ...
    const {data: profileRow, error: pErr} = await supabaseClient.from("profiles").select("id, username, media_perm").eq("username", u).maybeSingle();
    if (pErr) {
  ...
  async function cmdCheckHistory(u1, u2) {
    ...
    const {data: p1, error: e1} = await supabaseClient.from("profiles").select("id, username").eq("username", u1).maybeSingle();
    if (e1 || !p1) {
      termPrint("  ERR  no record for handle: " + u1, "err");
      termPrint("", "");
      return;
    ...
    const {data: p2, error: e2} = await supabaseClient.from("profiles").select("id, username").eq("username", u2).maybeSingle();
    if (e2 || !p2) {
      termPrint("  ERR  no record for handle: " + u2, "err");
      termPrint("", "");
      return;
    ...
    if (p1.id === p2.id) {
      termPrint("  ERR  both handles resolve to the same account", "err");
      termPrint("", "");
      return;
    }
    termPrint("  [3/3]  querying message history...", "info");
    const {data: msgs, error: mErr} = await supabaseClient.from("messages").select("id", {
      count: "exact"
    }).or(`and(sender_id.eq.${p1.id},receiver_id.eq.${p2.id}),and(sender_id.eq.${p2.id},receiver_id.eq.${p1.id})`).limit(1);
    if (mErr) {
      termPrint("  ERR  query fault — " + mErr.message, "err");
      termPrint("", "");
      return;
    }
  ...
  async function cmdClearAll(u) {
    ...
    const {data: profile, error: pErr} = await supabaseClient.from("profiles").select("id, username").eq("username", u).maybeSingle();
    if (pErr || !profile) {
      termPrint("  ERR  no record for handle: " + u, "err");
      termPrint("", "");
      return;
    ...
    const {data: msgs, error: mErr} = await supabaseClient.from("messages").select("sender_id, receiver_id").or(`sender_id.eq.${uid},receiver_id.eq.${uid}`);
    if (mErr) {
      termPrint("  ERR  query fault — " + mErr.message, "err");
      termPrint("", "");
      return;
