...
// Skeleton shown for the (rare) case the picker is opened before its DOM
// has finished building, so the popup never appears to "hang" empty.
if (!document.getElementById('emoji-picker-skeleton-style')) {
  const skelStyle = document.createElement('style');
  skelStyle.id = 'emoji-picker-skeleton-style';
  skelStyle.textContent = `
    .emoji-picker-skeleton { padding: 10px 12px; }
    .emoji-skel-tabs { display: flex; gap: 8px; margin-bottom: 10px; }
    .emoji-skel-tab { width: 24px; height: 24px; border-radius: 6px; }
    .emoji-skel-search { height: 34px; border-radius: 10px; margin-bottom: 10px; }
    .emoji-skel-grid { display: grid; grid-template-columns: repeat(7, 1fr); gap: 6px; }
    .emoji-skel-item { width: 100%; padding-top: 100%; border-radius: 6px; }
    .emoji-skel-tab, .emoji-skel-search, .emoji-skel-item {
      background: linear-gradient(90deg, rgba(127,127,127,0.12) 25%, rgba(127,127,127,0.22) 37%, rgba(127,127,127,0.12) 63%);
      background-size: 400% 100%;
      animation: emojiSkeletonShimmer 1.2s ease-in-out infinite;
    }
    @keyframes emojiSkeletonShimmer {
...
function buildEmojiPickerSkeleton() {
  const el = document.createElement('div');
  el.className = 'emoji-picker-skeleton';
  const tabs = Array(7).fill('<div class="emoji-skel-tab"></div>').join('');
  const items = Array(28).fill('<div class="emoji-skel-item"></div>').join('');
  el.innerHTML = `
    <div class="emoji-skel-tabs">${tabs}</div>
    <div class="emoji-skel-search"></div>
    <div class="emoji-skel-grid">${items}</div>
  `;
  return el;
}
...
function ensureEmojiPickerSkeleton() {
  ...
  // Enter while the emoji picker is open (focus sits in its search box) should
  // still send whatever draft is already in the message box, instead of being
  // swallowed by the search input with no effect.
  emojiSearchInputEl.addEventListener('keydown', (e) => {
    if (e.key !== 'Enter') return;
    e.preventDefault();
    const mode = emojiPickerMode;
    closeEmojiPicker();
    if (mode === 'flux') fluxSend('flux');
    else if (mode === 'mobile') fluxSend('mobile');
    else sendMessage();
  });
...
// Building the picker's DOM while it's display:none (see prebuild below)
// skips layout AND paint entirely, so the ~500 emoji glyphs never actually
// get shaped/rasterized until the real first open — which is what still
// made that first open feel slow despite the DOM being ready. This forces
// one throwaway render pass (opacity:0 instead of display:none, so the
// browser really paints it) immediately after prebuild, off-canvas and
// invisible, purely to warm the font/glyph cache before the user can click.
function warmUpEmojiPickerPaint() {
  const picker = document.getElementById('emojiPicker');
  if (!picker) return;
  picker.style.opacity = '0';
  picker.style.pointerEvents = 'none';
  picker.style.display = 'flex';
  picker.getBoundingClientRect(); // flush layout synchronously
...
// Focuses (and selects any leftover text in) the search box so the person
// can start typing to filter emoji the instant the picker is visible.
function focusEmojiSearchInput() {
  if (!emojiSearchInputEl) return;
  // Wait a frame so focus lands after the popup's own open transition/layout,
  // otherwise some browsers silently drop the focus call.
  requestAnimationFrame(() => {
    emojiSearchInputEl.focus({ preventScroll: true });
    emojiSearchInputEl.select();
  });
}
...
function toggleEmojiPicker(mode = 'relay') {
  ...
  // --- TEMP DIAGNOSTICS ---
  // Logs, in the browser console, how long it takes from the click to the
  // popup actually being painted on screen, split into the two stages that
  // could be slow: (1) applying the .show classes and (2) the next real
  // paint after that. Remove this block once the delay is tracked down.
  const __emojiOpenT0 = performance.now();
  console.log('[emoji-picker] click fired at', __emojiOpenT0.toFixed(1) + 'ms since navigation start, built =', emojiPickerBuilt);
  requestAnimationFrame(() => requestAnimationFrame(() => {
    console.log('[emoji-picker] click → next paint:', (performance.now() - __emojiOpenT0).toFixed(1) + 'ms');
  }));
  let triggerEl;
  ...
  // Open the popup instantly. If the real grid hasn't finished building yet
  // (e.g. the picker is opened a moment after page load, before the
  // prebuild below has run), show a skeleton immediately instead of leaving
  // the popup looking empty/frozen, then swap in the real content as soon
  // as it's ready.
  ...
  if (emojiPickerBuilt) {
    ...
    let skeleton = picker.querySelector('.emoji-picker-skeleton');
    if (!skeleton) {
      skeleton = buildEmojiPickerSkeleton();
      picker.appendChild(skeleton);
    }
...
function toggleFluxSendBtn() {
  ...
  // Both controls stay visible. Media never replaces the send button.
...
window.addEventListener('load', function() {
  toggleMediaButton();
  document.getElementById('fluxSendBtn')?.classList.remove('hidden');
...
(function scheduleEmojiPickerPrebuild() {
  const build = () => { initEmojiPicker(); warmUpEmojiPickerPaint(); };
  // Build immediately rather than deferring to an idle callback. The
  // ~500-button grid takes single-digit milliseconds, so there's no
  // real benefit to waiting, and doing so is what let the picker still
  // be unbuilt (and feel slow to open) if someone clicked the emoji
  // button shortly after the page loaded.
