function closeAllFluxDropdowns(skip) {
  const closers = [ typeof closeMsgActionMenu === "function" ? closeMsgActionMenu : null, typeof closeCtxMenu === "function" ? closeCtxMenu : null, typeof closeFluxConvMenu === "function" ? closeFluxConvMenu : null, typeof closeFluxHeaderMoreMenu === "function" ? closeFluxHeaderMoreMenu : null, typeof closeProfileDropdown === "function" ? closeProfileDropdown : null, typeof closeGroupAvatarMenu === "function" ? closeGroupAvatarMenu : null, typeof _closeFluxFolderKeyMenus === "function" ? _closeFluxFolderKeyMenus : null, typeof closeReactionPicker === "function" ? closeReactionPicker : null ];
  closers.forEach(fn => {
    if (fn && fn !== skip) {
      try {
        fn();
...
function toggleProfileDropdown() {
  ...
  closeAllFluxDropdowns(closeProfileDropdown);
  dropdown.style.left = "";
...
function addAccount() {
  closeProfileDropdown();
  alert("Add account flow — integrate your auth provider here.");
}
...
function openAvatarPicker() {
  document.getElementById("bannerAvatarUpload").click();
}
...
function initCropCanvas() {
  ...
  const boxSize = Math.round(wSize * .8);
  _cropBox = {
...
function positionCropUI() {
  const {x: x, y: y, w: w, h: h} = _cropBox;
  ...
  if (dimT) {
    dimT.style.cssText = `position:absolute;background:rgba(40,40,44,0.6);pointer-events:none;top:0;left:0;width:${S}px;height:${y}px;`;
  }
  if (dimB) {
    dimB.style.cssText = `position:absolute;background:rgba(40,40,44,0.6);pointer-events:none;top:${y + h}px;left:0;width:${S}px;height:${S - (y + h)}px;`;
  }
  if (dimL) {
    dimL.style.cssText = `position:absolute;background:rgba(40,40,44,0.6);pointer-events:none;top:${y}px;left:0;width:${x}px;height:${h}px;`;
  }
  if (dimR) {
    dimR.style.cssText = `position:absolute;background:rgba(40,40,44,0.6);pointer-events:none;top:${y}px;left:${x + w}px;width:${S - (x + w)}px;height:${h}px;`;
  }
...
function renderPreview() {
  ...
  ctx.drawImage(_cropImg, (_cropOffX - x) * scaleX, (_cropOffY - y) * scaleY, _cropImg.width * _cropScale * scaleX, _cropImg.height * _cropScale * scaleY);
}
...
function setupCropPointerEvents() {
  ...
  const onDown = e => {
    ...
    _cropDragOX = _cropOffX;
    _cropDragOY = _cropOffY;
  };
  const onMove = e => {
...
async function applyCrop() {
  ...
  const srcX = x, srcY = y;
  ctx.drawImage(_cropImg, (_cropOffX - srcX) * pxPer, (_cropOffY - srcY) * pxPer, _cropImg.width * _cropScale * pxPer, _cropImg.height * _cropScale * pxPer);
  off.toBlob(async blob => {
    closeCropModal();
    await uploadCroppedAvatar(new File([ blob ], "avatar.png", {
      type: "image/png"
    }));
    btn.disabled = false;
...
async function uploadCroppedAvatar(file) {
  ...
  if (existingProfile?.avatar_url) {
    try {
      const path = existingProfile.avatar_url.split("/avatars/")[1];
  ...
  const {data: data} = supabaseClient.storage.from("avatars").getPublicUrl(filePath);
...
async function removeAvatar() {
  const {data: {user: user}} = await supabaseClient.auth.getUser();
  if (!user) return;
  const {data: data} = await supabaseClient.from("profiles").select("avatar_url").eq("id", user.id).single();
  if (data?.avatar_url) {
    try {
...
document.addEventListener("keydown", e => {
  ...
  if (e.key === "Escape") {
    ...
    if (document.getElementById("vaultOverlay")?.classList.contains("show")) {
      closeVaultPopup();
      return;
    }
    if (document.getElementById("clearRelayOverlay").classList.contains("show")) {
