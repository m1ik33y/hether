// ── CLOSE ALL FLOATING DROPDOWNS / MENUS (mutual exclusivity) ──
// Call before opening any dropdown-style popup so only one can be open at
// a time. Pass the opener's own close function as `skip` so a toggle can
// still close itself on a second click instead of being force-reopened.
function closeAllFluxDropdowns(skip) {
  const closers = [
    typeof closeMsgActionMenu === 'function' ? closeMsgActionMenu : null,
    typeof closeCtxMenu === 'function' ? closeCtxMenu : null,
    typeof closeFluxConvMenu === 'function' ? closeFluxConvMenu : null,
    typeof closeFluxHeaderMoreMenu === 'function' ? closeFluxHeaderMoreMenu : null,
    typeof closeProfileDropdown === 'function' ? closeProfileDropdown : null,
    typeof closeGroupAvatarMenu === 'function' ? closeGroupAvatarMenu : null,
    typeof _closeFluxFolderKeyMenus === 'function' ? _closeFluxFolderKeyMenus : null,
    typeof closeReactionPicker === 'function' ? closeReactionPicker : null,
  ];
  closers.forEach(fn => {
    if (fn && fn !== skip) { try { fn(); } catch (e) {} }
  });
}
