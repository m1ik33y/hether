...
const subscriptionManager = {
  ...
  // Set to true for the duration of a teardown we initiated ourselves
  // (switching conversations, closing the panel, etc.). The message
  // channel's own status callback checks this flag so it never mistakes
  // our own intentional removeChannel() call for a real network drop and
  // tries to "reconnect" a channel we're deliberately replacing.
...
// Startup playback is intentionally lazy: the app hydrates only the locally
// remembered last-played song. The full songs table is fetched only when a
// music-heavy page (Library / Explore) is opened, or when playback needs the
// next/previous song.
...
// Fetch exactly one song for playback navigation. This deliberately does NOT
// fetch the library list or any storage object. Supabase only returns the row;
// the browser downloads the thumbnail/audio only after we actually use its URL.
async function fetchLazyAdjacentSong(direction) {
  ...
  try {
    ...
    // The localStorage snapshot intentionally contains only what the player
    // needs at startup. Resolve created_at only when the user actually asks
    // for previous/next.
    if (!currentCreatedAt) {
      const { data: currentRow, error: currentErr } = await libraryClient
        .from('songs')
        .select('id, created_at')
        .eq('id', libCurrentSong.id)
        .maybeSingle();
    ...
    let query = libraryClient
      .from('songs')
      .select('id,title,artist,thumbnail_url,music_url,file_path,duration_seconds,created_at')
      .limit(1);
    ...
    // Match the existing player behavior: sequential playback wraps at the
    // end/start. Shuffle picks a single random song only when requested.
    if (!data || !data.length) {
      const { data: wrapped, error: wrapErr } = await libraryClient
        .from('songs')
        .select('id,title,artist,thumbnail_url,music_url,file_path,duration_seconds,created_at')
...
// Applies one lazily fetched song to the player. No library-wide cache is
// populated, so opening the app remains cheap even for a large library.
...
// Loads the one song needed for the next/previous action, then immediately
// starts it. This is the only path that pulls another song while staying in
// the lightweight startup mode.
...
function updateNpQueueCard() {
  ...
  // Do not fetch anything just to populate the queue preview at startup.
  // The next song is fetched only when the user actually requests it.
...
async function playNextLibSong() {
  if (!libLoaded && libCurrentSong) {
    if (libPlayMode === 'shuffle') {
      // Shuffle still loads exactly one row; we avoid downloading the whole
      // library just to choose a random item.
      const { count, error: countErr } = await libraryClient
        .from('songs')
...
function updateNowPlayingPanelVisibility(view) {
  ...
  // Profile keeps it, matching library/favourites. Aloft also keeps it so
  // the now-playing tab stays visible on the right while playing the game.
