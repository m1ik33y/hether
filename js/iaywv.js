...
// ── INPUT ──
// Any popup/overlay that takes typed input (e.g. the admin key / vault
// popup) must win over the game's global key capture. Without this check,
// onAloftKey (bound on window) intercepts every keydown before it can be
// typed normally, and preventDefault()-ing Space/A/D/arrows here stops
// those characters from ever reaching a focused text field.
function _aloftInputIsCapturingKeys() {
  ...
  // Also bail out if the admin key / vault overlay is open, even if focus
  // hasn't landed on the input yet (e.g. right after it was opened).
...
function onAloftKey(e) {
  if (_aloftInputIsCapturingKeys()) return; // let the popup/input have the key
  keys[e.code] = true;
