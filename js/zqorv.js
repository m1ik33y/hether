...
function _fluxHidePinnedBar(messagesId) {
  const msgsEl = document.getElementById(messagesId);
  const bar = msgsEl?.parentElement?.querySelector('.flux-pinned-messages-bar');
...
function _fluxClearPinnedFrontend() {
  ...
  _fluxHidePinnedBar('fluxRelayMessages');
  _fluxHidePinnedBar('fluxFsMessages');
}

function _fluxPinnedMessagePreview(msg) {
...
async function _fluxGetPinnedMessages(convId) {
  if (!convId) return [];
  const { data: { user } } = await supabaseClient.auth.getUser();
  if (!user) return [];
  const isGroup = _fluxConvIsGroup(convId);

  const { data: pins, error: pinError } = await supabaseClient
  ...
  return (pins || [])
    .map(p => {
      const msg = byId.get(p.message_id);
      return msg ? { ...msg, _pinCreatedAt:p.created_at, _pinnedBy:p.pinned_by } : null;
    })
    .filter(msg => msg && _fluxMsgBelongsToConv(msg, convId, user.id, isGroup));
}

function _fluxPinnedVisibleMessagesElement() {
  const ids = ['fluxRelayMessages','fluxFsMessages'];
  let fallback = null;
  for (const id of ids) {
...
async function _fluxLoadPinnedTargetMessage(messageId) {
  if (!messageId) return false;

  const alreadyThere = document.querySelector(`[data-msg-id="${CSS.escape(String(messageId))}"]`);
  if (alreadyThere) return true;

  const msgsEl = _fluxPinnedVisibleMessagesElement();
  if (!msgsEl) return false;

  const isDesktop = msgsEl.id === 'fluxRelayMessages';
  const maxAttempts = 100;

  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    const found = msgsEl.querySelector(`[data-msg-id="${CSS.escape(String(messageId))}"]`);
    if (found) return true;

    const allLoaded = isDesktop ? fluxDesktopAllLoaded : fluxMobileAllLoaded;
    const loading = isDesktop ? fluxDesktopLoadingMore : fluxMobileLoadingMore;
    if (allLoaded) break;
    ...
    if (isDesktop) await loadMoreMessagesDesktop();
    else await loadMoreMessagesMobile();

    await new Promise(r => setTimeout(r, 20));
  }

  return !!msgsEl.querySelector(`[data-msg-id="${CSS.escape(String(messageId))}"]`);
}

async function _fluxJumpToPinnedMessage(messageId) {
  if (!messageId) return;

  let target = document.querySelector(`[data-msg-id="${CSS.escape(String(messageId))}"]`);
  if (!target) {
    const loaded = await _fluxLoadPinnedTargetMessage(messageId);
    if (!loaded) return;
    target = document.querySelector(`[data-msg-id="${CSS.escape(String(messageId))}"]`);
  }
...
function _fluxRenderPinnedMessages(convId, pins, messagesId) {
  ...
  _fluxPinnedMessageIds.clear();
  _fluxPinnedItems.forEach(msg => _fluxPinnedMessageIds.add(msg.id));

  inner.innerHTML = '';
  bar.querySelector('.flux-pinned-card-count')?.remove();
  bar.querySelector('.flux-pinned-unpin')?.remove();
  bar.querySelector('.flux-pinned-next')?.remove();
  ...
  const msg = _fluxPinnedItems[_fluxPinnedIndex];
  const card = document.createElement('button');
  card.type = 'button';
  card.className = 'flux-pinned-card';
  card.title = 'Jump to pinned message';
  card.innerHTML = `
    <span class="flux-pinned-card-icon"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-pin-icon lucide-pin"><path d="M12 17v5"/><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V7a1 1 0 0 1 1-1 2 2 0 0 0 0-4H8a2 2 0 0 0 0 4 1 1 0 0 1 1 1z"/></svg></span>
    <span class="flux-pinned-card-copy">
      <span class="flux-pinned-card-text">${escHtml(_fluxPinnedMessagePreview(msg))}</span>
    </span>`;
  card.onclick = () => _fluxJumpToPinnedMessage(msg.id);
  inner.appendChild(card);

  const count = document.createElement('span');
  count.className = 'flux-pinned-card-count';
  count.textContent = `${_fluxPinnedIndex + 1}/${_fluxPinnedItems.length}`;
  bar.appendChild(count);

  const unpin = document.createElement('button');
  unpin.type = 'button';
  unpin.className = 'flux-pinned-unpin';
  unpin.title = 'Unpin message';
  unpin.setAttribute('aria-label', 'Unpin message');
  unpin.innerHTML = '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-pin-off-icon lucide-pin-off"><path d="M12 17v5"/><path d="M15 9.34V7a1 1 0 0 1 1-1 2 2 0 0 0 0-4H7.89"/><path d="m2 2 20 20"/><path d="M9 9v1.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a1 1 0 0 0 1 1h11"/></svg>';
  ...
  if (_fluxPinnedItems.length > 1) {
    const next = document.createElement('button');
    next.type = 'button';
    next.className = 'flux-pinned-next';
    next.title = 'Next pinned message';
    next.setAttribute('aria-label', 'Next pinned message');
    next.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"/><path d="m12 5 7 7-7 7"/></svg>';
    next.onclick = e => {
      e.preventDefault();
      e.stopPropagation();
      _fluxPinnedIndex = (_fluxPinnedIndex + 1) % _fluxPinnedItems.length;
      _fluxRenderPinnedMessages(convId, _fluxPinnedItems, messagesId);
    };
...
async function toggleFluxMessagePin(msgId){
  ...
  await loadFluxPinnedMessages(activeFluxId,'fluxFsMessages');
}
...
async function loadNicknameSideTab(targetId) {
  ...
  await loadFluxDisappearingState(targetId);
...
function openGroupAvatarMenu(e) {
  ...
  e.stopPropagation();
  closeAllFluxDropdowns(closeGroupAvatarMenu);
...
async function loadGroupInfoSideTab(groupId, myUserId) {
  ...
  _updateFluxDisappearingProfileButton(groupId);
