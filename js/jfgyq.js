...
// ── DM DISAPPEARING MESSAGES (24 HOURS) ────────────────────────────────
// Shared per-DM setting stored on the same sorted user pair as chat_settings.
// The database migration included with this build stamps expires_at on every
// new DM message while the switch is enabled, then pg_cron deletes it server-
// side exactly from its created_at timestamp. The UI cache is only for display.
const _fluxDisappearingMessages = new Map();
const FLUX_DISAPPEARING_ICON = '<circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/><path d="M4.9 4.9 3 3"/>';

function _fluxDisappearingOf(convId) {
...
function _updateFluxDisappearingProfileButton(convId) {
  ...
  if (!btn) return;
  const isGroup = !convId || _fluxConvIsGroup(convId);
  btn.style.display = isGroup ? 'none' : '';
  if (state && !isGroup) state.textContent = _fluxDisappearingOf(convId) ? 'On' : 'Off';
  btn.classList.toggle('active', !isGroup && _fluxDisappearingOf(convId));
}

async function loadFluxDisappearingState(convId) {
  if (!convId || _fluxConvIsGroup(convId)) {
    _updateFluxDisappearingProfileButton(convId);
    return false;
  }
  const { data: { user } } = await supabaseClient.auth.getUser();
  if (!user) return false;
  const [u1, u2] = _sortUserIds(user.id, convId);
  const { data, error } = await supabaseClient
    ...
    .maybeSingle();
  if (error) {
    console.warn('[FLUX] disappearing-message state load failed:', error.message || error);
    return _fluxDisappearingOf(convId);
  }
  const enabled = !!data?.disappearing_messages;
  _fluxDisappearingMessages.set(convId, enabled);
  _updateFluxDisappearingProfileButton(convId);
  return enabled;
}

async function toggleFluxDisappearingMessages(convId) {
  convId = convId || activeFluxProfileTarget || activeFluxId;
  if (!convId || _fluxConvIsGroup(convId)) return;
  const { data: { user } } = await supabaseClient.auth.getUser();
  if (!user) return;
  const current = await loadFluxDisappearingState(convId);
  const next = !current;
  const [u1, u2] = _sortUserIds(user.id, convId);
  const { error } = await supabaseClient
    ...
    .upsert({
      user1_id: u1,
      user2_id: u2,
      disappearing_messages: next,
      changed_by: user.id
    }, { onConflict: 'user1_id,user2_id' });
  if (error) {
    console.error('[FLUX] disappearing-message toggle failed:', error);
    return;
  }
  _fluxDisappearingMessages.set(convId, next);
  _updateFluxDisappearingProfileButton(convId);
  if (typingChannel && typingChannel.state === 'joined') {
    typingChannel.send({
      type: 'broadcast',
      event: 'disappearing_messages_change',
      payload: { enabled: next, senderId: user.id, convId }
    });
  }

  // Keep both participants informed with a concise system message.
  // Use the actor's username so both participants see who changed the setting.
  let myUsername = 'Someone';
  try {
    const { data: meProfile } = await supabaseClient
      ...
      .eq('id', user.id)
      .maybeSingle();
    myUsername = meProfile?.username || 'Someone';
  } catch (e) {
    console.warn('[FLUX] disappearing-message username lookup failed:', e.message || e);
  }

  await _fluxPostSystemMessage(
    convId,
    next
      ? `${myUsername} turned on disappearing message.`
      : `${myUsername} turned off disappearing message.`
  );
}
...
const _fluxPinnedMessageIds = new Set();

function showMsgActionMenu(e, bWrap, msg, isSent) {
  e.stopPropagation();
  closeAllFluxDropdowns(closeMsgActionMenu);
  const menu = document.getElementById('fluxMsgActionMenu');
  menu.innerHTML = '';
  const text = bWrap.dataset.text || '';
  const sender = bWrap.dataset.sender;

  const replyBtn = document.createElement('button');
  replyBtn.className = 'flux-msg-action-item';
  replyBtn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polyline points="9 17 4 12 9 7"/><path d="M20 18v-2a4 4 0 0 0-4-4H4"/></svg> Reply`;
  replyBtn.onclick = (ev) => { ev.stopPropagation(); closeMsgActionMenu(); startReply(text, sender, msg ? msg.id : null); };
  ...
  reactBtn.className = 'flux-msg-action-item';
  reactBtn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-face-slightly-smiling-plus-icon lucide-face-slightly-smiling-plus"><path d="M13.267 2.08a10 10 0 108.653 8.653"/><path d="M15 10V9"/><path d="M16 5h6"/><path d="M16.472 15a6 6 0 01-8.943 0"/><path d="M19 2v6"/><path d="M9 10V9"/></svg> React`;
  reactBtn.onclick = (ev) => { ev.stopPropagation(); closeMsgActionMenu(); showReactionPicker(bWrap); };
  menu.appendChild(reactBtn);

  // Pin / unpin this individual message. This is separate from conversation
  // pinning above and is limited to 3 messages per conversation by the DB.
  if (msg && msg.id) {
    const pinBtn = document.createElement('button');
    pinBtn.className = 'flux-msg-action-item';
    const pinIcon = '<path d="M12 17v5"/><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V7a1 1 0 0 1 1-1 2 2 0 0 0-2-4H8a2 2 0 0 0 0 4 1 1 0 0 1 1 1z"/>';
    pinBtn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round">${pinIcon}</svg> ${_fluxPinnedMessageIds.has(msg.id) ? 'Unpin' : 'Pin'}`;
    pinBtn.onclick = async (ev) => {
      ev.stopPropagation();
      closeMsgActionMenu();
      await toggleFluxMessagePin(msg.id);
    };
  ...
  if (isSent && msg && msg.id) {
    if (bWrap.dataset.text && bWrap.dataset.text.trim()) {
      ...
      editBtn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-bolt-icon lucide-bolt"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/><circle cx="12" cy="12" r="4"/></svg> Edit`;
      editBtn.onclick = (ev) => { ev.stopPropagation(); closeMsgActionMenu(); editMessage(msg.id, bWrap); };
      menu.appendChild(editBtn);

      const divider = document.createElement('div');
      divider.className = 'flux-msg-action-divider';
      menu.appendChild(divider);
    }
    const deleteBtn = document.createElement('button');
    deleteBtn.className = 'flux-msg-action-item danger';
    deleteBtn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/></svg> Delete`;
    deleteBtn.onclick = async (ev) => { ev.stopPropagation(); closeMsgActionMenu(); await unsendMessage(msg.id); };
...
function openFluxHeaderMoreMenu(e) {
  ...
  closeAllFluxDropdowns(closeFluxHeaderMoreMenu);
  ...
  if (isGroup) {
    ...
    // Pin and move are available from the open-chat header too, using the
    // exact same icons as the chevron conversation dropdown.
    if (!_fluxArchivedOf(id)) {
      menu.appendChild(makeItem(
        _fluxPinnedOf(id) ? 'Unpin' : 'Pin',
        _fluxPinnedOf(id)
          ? '<path d="M12 17v5"/><path d="M15 9.34V6a1 1 0 0 1 1-1 2 2 0 0 0 0-4H9.31"/><path d="M16.13 16.13A2 2 0 0 1 15 17H5a1 1 0 0 1-.71-1.71l.87-.87A2 2 0 0 0 6 13.03V10a2 2 0 0 1 .61-.92"/><line x1="2" x2="22" y1="2" y2="22"/>'
          : '<path d="M12 17v5"/><path d="M9 10.76a2 2 0 0 1-1.11 1.79l-1.78.9A2 2 0 0 0 5 15.24V16a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-.76a2 2 0 0 0-1.11-1.79l-1.78-.9A2 2 0 0 1 15 10.76V7a1 1 0 0 1 1-1 2 2 0 0 0-2-4H8a2 2 0 0 0 0 4 1 1 0 0 0 1 1z"/>',
        () => toggleFluxPin(id)
      ));
    }
    if (_fluxCategoryOf(id) === 'primary') {
      menu.appendChild(makeItem('Move to General',
        '<rect x="3" y="4" width="18" height="18" rx="2"/><path d="M3 10h18"/>',
        () => moveFluxContact(id, 'general')));
    } else {
      menu.appendChild(makeItem('Move to Primary',
        '<path d="m3 11 18-5v12L3 14v-3z"/><path d="M11.6 16.8a3 3 0 1 1-5.8-1.6"/>',
        () => moveFluxContact(id, 'primary')));
    }
    ...
    // Pin / unpin the conversation. Keep the exact same pin icon used by
    // the chevron conversation dropdown.
    if (!isArchived) {
      menu.appendChild(makeItem(
        _fluxPinnedOf(id) ? 'Unpin' : 'Pin',
        _fluxPinnedOf(id)
    ...
    // Keep the same move icons used by the chevron dropdown, directly above
    // Clear all as requested.
...
function closeConfirm() {
  ...
  document.getElementById('confirmPopup')?.classList.remove('hether-download-popup');
...
function handleDownloadClick() {
  ...
  popup?.classList.add('hether-download-popup');
  showConfirm('Download Hether', 'Download Hether in your PC?', 'Download', () => {
    popup?.classList.remove('hether-download-popup');
...
function toggleFluxFolderKeyMenu(e) {
  ...
  closeAllFluxDropdowns(_closeFluxFolderKeyMenus);
