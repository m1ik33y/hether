// Points at whichever of _setupDesktopMsgChannel / _setupMobileMsgChannel
// is currently in charge of the message channel. Each channel already
// reconnects itself via its .subscribe() status callback the instant the
// socket drops, but this is kept as a fallback for _rehydrateRealtime()
// (asrng.js) in case a status change is ever missed.
    ...
    const channel = supabaseClient.channel(`messages:${user.id}:${userId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, (payload) => {
        ...
        if (activeFluxId === userId) {
          ...
          // NOTE: no "else mark unread" branch here on purpose. This block
          // only runs when activeFluxId === userId — i.e. this conversation
          // is the one currently open. If the tab is merely backgrounded
          // (not visible/focused), the message still isn't "seen" server-
          // side yet, but it should NOT show a dot/count either — the user
          // is already on this chat, just on another tab. The sound above
          // is what notifies them in that case; the dot is reserved for
          // conversations the user hasn't actually got open. Previously this
          // fell into an else branch that set contact.unread/unreadCount,
          // which both showed a stale dot on an already-open chat AND kept
          // incrementing unreadCount every background message without ever
      ...
      .subscribe((status, err) => {
        if ((status === 'CLOSED' || status === 'CHANNEL_ERROR' || status === 'TIMED_OUT')
            && !subscriptionManager._intentionalTeardown
            && fluxOpen && activeFluxId === userId) {
          // The websocket dropped — almost always because the tab was
          // backgrounded/throttled by the browser, not because anything
          // about the conversation changed. Just rebuild this one channel
          // and resubscribe; don't touch the message list or refetch
          // anything. This is what used to trigger a full openDesktopRelay()
          // reload on every tab refocus.
    ...
    const channel = supabaseClient.channel(`fs-messages:${user.id}:${userId}`)
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'messages' }, (payload) => {
        ...
        if (activeFluxId === userId) {
          ...
          // See matching comment in loadMessages() (desktop) — intentionally
      ...
      .subscribe((status, err) => {
        if ((status === 'CLOSED' || status === 'CHANNEL_ERROR' || status === 'TIMED_OUT')
            && !subscriptionManager._intentionalTeardown
            && fluxOpen && activeFluxId === userId) {
          // Same reasoning as the desktop channel in loadMessages(): the
          // socket dying from a backgrounded tab isn't a reason to reload
          // the conversation — just reconnect this one channel.
...
async function openFsRelay(id) {
  ...
  if (!_fluxConvIsGroup(id)) loadFluxDisappearingState(id);
...
async function fluxSend(mode) {
  ...
  // Render all staged media immediately; each item is uploaded/inserted separately below.
  // The typed caption is persisted (below) onto the LAST staged media item, so that
  // item needs to render as a single wrapped image+caption bubble right away —
  // otherwise it shows as a separate media bubble plus a separate text bubble
  // that only visually merge into one after a reload re-fetches the row.
  const mediaLoadingRemovers = [];
  let captionBubbleRefs = null; // { bWrap, bubble } for the media item carrying the caption, if any
  staged.forEach((media, idx) => {
    ...
    if (isLastMedia && text) {
      ...
      mediaLoadingRemovers.push(
        appendFluxMediaBubble(msgsEl, media.dataUrl, media.isVideo, user ? user.id : null, contact)
      );
    }
  });
  if (mode === 'mobile') clearFluxFsStaging(); else clearFluxStaging();
  ...
  if (captionBubbleRefs) {
    // Caption is already rendered onto the last media bubble above — reuse
    // those refs instead of building a second, separate text bubble.
    ...
    // Only show optimistic text bubble if there's actual text (and it isn't
    // already attached to a media caption bubble handled above)
    if (!text) {
  ...
  try {
    ...
    // Insert each staged media as its own message, while attaching the typed text
    // only to the final message (or creating a text-only message when there is no media).
    ...
    // If there was text without media, preserve the normal single-message behavior.
    ...
    if (inserted?.id) {
      ...
      if (moreBtnOpt) moreBtnOpt.onclick = (e) => { e.stopPropagation(); showMsgActionMenu(e, bWrap, inserted, true); };
    }
    buildFLUXConvList();
    mediaLoadingRemovers.forEach(fn => { try { fn?.(); } catch (e) {} });
...
/* Multi-media Flux staging: each thumbnail has its own hover remove control. */
...
function _setFluxStaged(mode, value) {
  if (mode === 'mobile') fluxFsStagedMedia = value;
  else fluxStagedMedia = value;
}
...
function showFluxErrorToast(message, title = 'Error') {
  ...
  el.innerHTML = '<div style="font-weight:600;">' + escHtml(title) + '</div>' +
    '<div style="margin-top:4px;color:#c9c9c9;">' + escHtml(message) + '</div>';
...
// Same idea as appendFluxMediaBubble, but for a media item sent together with a
// typed caption. Builds the media + caption as ONE bubble (.flux-bubble-media-caption)
// right away, matching how it renders after a reload re-fetches the row — instead of
// a separate media-only bubble sitting next to a separate text bubble.
function appendFluxMediaCaptionBubble(container, dataUrl, isVideo, captionText, currentUserId, contact) {
  ...
  // Loading indicator, same as plain media bubbles, while upload is in flight
  const loadingEl = document.createElement('div');
  loadingEl.className = 'flux-media-loading';
  loadingEl.innerHTML = `<div class="flux-sending-spinner"></div>`;
  ...
  if (_typSlotMediaCap) { container.insertBefore(groupEl, _typSlotMediaCap); } else { container.appendChild(groupEl); }
  pinTypingIndicator(container);
  container.scrollTop = container.scrollHeight;
  return {
    bWrap,
    bubble,
    removeLoading: () => { loadingEl.remove(); bubble.classList.remove('sending'); }
  };
}
