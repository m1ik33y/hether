...
async function joinPresence() {
  ...
  presenceChannel.subscribe(async (status) => {
    if (status === 'SUBSCRIBED') {
      // Small delay before the first ping: right when SUBSCRIBED fires,
      // channel.state can still briefly read something other than 'joined'
      // internally, which makes supabase-js fall back to an HTTP POST to
      // /realtime/v1/api/broadcast instead of using the socket. Waiting a
      // tick lets state settle so this (and every refocus reconnect) sends
      // over the socket instead of firing an extra HTTP request.
      setTimeout(() => {
...
async function setupTypingChannel(myUserId, otherUserId) {
  ...
  typingChannel
    ...
    .on('broadcast', { event: 'disappearing_messages_change' }, ({ payload }) => {
      if (payload.senderId === myUserId) return;
      const convId = payload.convId || otherUserId;
      _fluxDisappearingMessages.set(convId, !!payload.enabled);
      if (activeFluxProfileTarget === convId || activeFluxId === convId) {
...
async function confirmClearRelay() {
  ...
  // Pins belong to messages, so remove the conversation's pin rows first.
  // Also clear the pinned bar immediately so it never survives the clear/load cycle.
  ...
  try {
    ...
    if (pinMessageIds.length) {
      const { error: pinDeleteError } = await supabaseClient
        .from('pinned_messages').delete().in('message_id', pinMessageIds);
